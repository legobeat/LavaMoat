...
var plugin = (() => {
  ...
  var require_report = __commonJS({
    "../allow-scripts/src/report.js"(exports, module) {
      "use strict";
      var { loadAllPackageConfigurations: loadAllPackageConfigurations2 } = require_setup();
  ...
  var isPackageScriptAllowed = async (project, npm_package_json) => {
    ...
    const {
      canonicalNamesByPath,
      configs: { lifecycle }
    } = await (0, import_setup.loadAllPackageConfigurations)({
  ...
  var plugin = {
    hooks: {
      ...
      afterAllInstalled: (project) => {
        (0, import_setup.loadAllPackageConfigurations)({
          rootDir: yarnCwdToPath(project.cwd)
        }).then((result) => {
          const {
            configs: { lifecycle },
